...
class Session {
	...
	public function __construct() {
		...
		if( $this->use_php_sessions ) {
			...
			if ( ! defined( 'WP_SESSION_COOKIE' ) ) {
				define( 'WP_SESSION_COOKIE', 'ss_wp_session' );
			}

			if ( ! class_exists( '\Recursive_ArrayAccess' ) ) {
				require_once SS_PLUGIN_PATH . 'includes/libraries/class-recursive-arrayaccess.php';
			}

			if ( ! class_exists( '\WP_Session' ) ) {
				require_once SS_PLUGIN_PATH . 'includes/libraries/class-wp-session.php';
				require_once SS_PLUGIN_PATH . 'includes/libraries/wp-session.php';
			}
	...
	public function init() {
		...
		if( $this->use_php_sessions ) {
			$this->session = isset( $_SESSION['ss' . $this->prefix ] ) && is_array( $_SESSION['ss' . $this->prefix ] ) ? $_SESSION['ss' . $this->prefix ] : array();
		} else {
	...
	public function set( $key, $value ) {
		...
		if( $this->use_php_sessions ) {
			...
			$_SESSION['ss' . $this->prefix ] = $this->session;
		}
	...
	public function use_php_sessions() {
		...
		$edd_use_php_sessions = (bool) get_option( 'ss_use_php_sessions' );
		...
		if ( ! $edd_use_php_sessions ) {
			...
			if( function_exists( 'session_start' ) ) {
				...
				$this->set( 'ss_use_php_sessions', 1 );

				if( $this->get( 'ss_use_php_sessions' ) ) {

					$ret = true;

					// Set the database option
					update_option( 'ss_use_php_sessions', true );
		...
		if ( defined( 'SS_USE_PHP_SESSIONS' ) && SS_USE_PHP_SESSIONS ) {
			$ret = true;
		} else if ( defined( 'SS_USE_PHP_SESSIONS' ) && ! SS_USE_PHP_SESSIONS ) {
			$ret = false;
		}

		return (bool) apply_filters( 'ss_use_php_sessions', $ret );
	}
	...
	public function should_start_session() {
		...
		return apply_filters( 'ss_start_session', $start_session );
	...
	public function get_blacklist() {
		...
		$blacklist = apply_filters( 'ss_session_start_uri_blacklist', array(
			'feed',
			'feed/rss',
			'feed/rss2',
			'feed/rdf',
